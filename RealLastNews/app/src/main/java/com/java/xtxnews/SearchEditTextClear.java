...
public class SearchEditTextClear extends AppCompatEditText {
    /**
     * 步骤1：定义左侧搜索图标 & 一键删除图标
     */
    private Drawable clearDrawable,searchDrawable;

    public SearchEditTextClear(Context context) {
        super(context);
        init();
        // 初始化该组件时，对EditText_Clear进行初始化 ->>步骤2
    }

    public SearchEditTextClear(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SearchEditTextClear(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    private void init() {
        clearDrawable = getContext().getDrawable(R.drawable.clear);
        searchDrawable = getContext().getDrawable(R.drawable.search);
